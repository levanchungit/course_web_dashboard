...
export function Video() {
  ...
  const fetchDataVideoById = async (_id) => {
    try {
      const data = await getVideo(_id);
      if (data) {
        return data;
      }
    } catch (error) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [categoriesData, videoDetailData] = await Promise.all([
        fetchDataCategories(),
        _id ? fetchDataVideoById(_id) : Promise.resolve(null), // Thay _id bằng giá trị tương ứng
      ...
      if (videoDetailData) {
        setTitle(videoDetailData.result.title);
        setDescription(videoDetailData.result.description);
        setContent(videoDetailData.result.content);
        setCoverImage(videoDetailData.result.cover_image);
        setStatus(videoDetailData.result.status);
        setPublishAt(videoDetailData.result.publish_at);
        setItems(videoDetailData.result.items);
        setStatusComment(videoDetailData.result.statusComment);
        setIdPlaylist(videoDetailData.result.idPlaylist);
        
        // Tạo một mảng danh mục mới với trạng thái checked dựa trên danh sách đã chọn
        const updatedCategories = categories.map((category) => ({
          ...category,
          checked: videoDetailData.result.categories.some(
